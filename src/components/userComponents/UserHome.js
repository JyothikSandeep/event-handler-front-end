...
export const UserHome = () => {
  ...
  useEffect(() => {
    async function getData() {
      const d = await axios.get("http://localhost:5000/admin/pastevents");
      console.log(d);
      setData(d.data);
    }
    getData();
  ...
  return (
    <div className="m-10">
      <div className="flex ">
        <div className="ongoing-events w-3/4 border border-gray-200 p-3">
          Ongoing
        </div>
        <div className="upcoming-events w-1/4 border border-gray-200 p-3 ">
          Upcoming events
        </div>
      </div>
      <div className="past-events border border-gray-200 p-3 ">
        <>
          <h1 className="text-2xl text-center mt-8">Past Events</h1>
                ...
                <div className="border border-gray-300 p-4 rounded-md shadow-md bg-white hover:shadow-xl">
                  ...
                  <div className="flex justify-around">
                    <p>
                      {new Date(d.event_date).getDate() +
                        "- " +
                        new Date(d.event_date).getMonth() +
                        "- " +
                        new Date(d.event_date).getFullYear()}
                    </p>
                    <p>{d.event_time}</p>
                  </div>
                  ...
                  <div className="text-center mt-3">
                    <button
                      ...
                      onClick={() => navigate("/eventdisplay", { state: d })}
                    >
                      View details
                    </button>
                  </div>
